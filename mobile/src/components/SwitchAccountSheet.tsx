...
export default function SwitchAccountSheet({ visible, current, onClose, onSelect }: { visible: boolean; current: SwitchAccountType; onClose: () => void; onSelect: (type: SwitchAccountType) => void }) {
  ...
  return <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
    <View style={{ flex: 1, justifyContent: "flex-end", backgroundColor: "rgba(0,0,0,0.45)" }}>
      ...
      <View style={{ backgroundColor: colors.sheet, borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 24, paddingBottom: Math.max(insets.bottom, 24), maxWidth: 620, width: "100%", alignSelf: "center", maxHeight: "85%" }}>
        ...
        <Text style={{ color: colors.textMuted, fontFamily: "Montserrat_400Regular", marginBottom: 18 }}>Choose the account you want to use.</Text>
        <ScrollView>{([ ["personal", "Personal", "person-outline", "Your community, shopping and wallet."], ["business", "Business", "briefcase-outline", "Your shop, listings and business tools."], ["agent", "Agent", "bicycle-outline", "Explore the agent preview."] ] as const).filter(([id]) => id !== current).map(([id, label, icon, description]) => <Pressable key={id} accessibilityRole="button" onPress={() => onSelect(id)} style={{ flexDirection: "row", alignItems: "center", gap: 14, backgroundColor: colors.card, borderRadius: 18, padding: 18, marginBottom: 12 }}><Ionicons name={icon} size={25} color={colors.primary} /><View style={{ flex: 1, gap: 5 }}><Text style={{ fontFamily: "Montserrat_700Bold", color: colors.text }}>{label}</Text><Text style={{ fontFamily: "Montserrat_400Regular", fontSize: 12, color: colors.textMuted }}>{description}</Text></View><Ionicons name="chevron-forward" size={18} color={colors.textMuted} /></Pressable>)}</ScrollView>
      </View>
    </View>
  </Modal>;
}
